...
class GlobalConfig:
    ...
    BIG_THRESHOLD: int = 5
    HISTORY_LEN: int = 10
    API_ENDPOINT: str = "https://draw.ar-lottery01.com/WinGo/WinGo_30S/GetHistoryIssuePage.json"
    API_RETRIES: int = 3
...
class WingoResultStream:
    ...
    def _get_current_round_start(self):
        return math.floor(time.time() / 30) * 30
    def fetch(self):
        try:
...
def real_time_loop():
    ...
    while True:
        now = time.time()
        current_round = math.floor(now/30)*30
        next_round = current_round + 30
